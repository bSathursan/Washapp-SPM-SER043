...
const PaymentList = () => {
    ...
    return (
            <div className=" relative">
                ...
                <Profile Img={ManagerImg} name="Balaputhiran Sathursan" role="Manager" />
                </div>
                <div className=" rounded-lg absolute right-24 top-5 inline-block shadow-xl p-10" style={{background:"#fafafa"}}>
                    <span style={{color:"#015958", fontSize:"20px", fontWeight:"600"}}>Total Income</span><br/>
                    <span className="text-3xl font-semibold">LKR {Total}.00</span>
                </div>
                ...
                <form onSubmit={getResults} className=" w-85 mt-5 ml-40  text-center relative mb-5">
                        ...
                        <button className="text-white absolute right-0 top-0 h-full rounded-2xl w-14 flex items-center justify-center font-bold text-2xl" style={{background:"#023535"}}>
                    ...
                    {NoResults ? (
                        <div>
                            <h1 className="text-2xl font-bold pb-5 ml-56 " style={{color:"#015958"}}>No Payment Found</h1>
                        ...
                        <div>{PaymentDetails.map((id)=>{
                            return(<div key={id.name} className=" ml-3 mb-5 mt-2 rounded-lg p-5 px-8 relative shadow-xl w-11/12" style={{background:"#fafafa"}}>
                                <span className="  font-medium" style={{color:"#015958", fontSize:"20px"}}>{id.name}</span><br/>
                                <span className="text-light-blue font-medium" style={{color:"#0FC2C0"}}>{id.date}</span>
                                <span className=" absolute right-8 top-7 text-2xl font-semibold" style={{color:"#008F8C"}}>LKR {id.amount}.00</span>
                            </div>)})}
                        </div>)
                    }
                    </div>
                </div>
                <button onClick={generateReport} className="rounded-full absolute right-24 top-100 text-white  font-semibold text-xl py-3 px-5" style={{background:"#023535"}}>Generate Report</button>
            </div>
    )
}
